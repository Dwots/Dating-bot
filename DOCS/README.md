...
## Важно: перевыпустить токен Telegram-бота

Текущий `BOT_TOKEN` был показан вне локального `.env`, поэтому его нужно считать раскрытым. При первой возможности перевыпусти токен через `@BotFather`, обнови значение `BOT_TOKEN` в `.env` и перезапусти сервисы, которые используют Telegram API:

```bash
docker compose up -d --force-recreate bot celery_worker celery_beat match_consumer
```

Реальный токен не должен попадать в README, логи, сообщения, скриншоты или Git. В репозитории оставляй только пример в `.env.example`.

## Полезные документы

- [technology-score.md](technology-score.md) — где используются технологии, сколько баллов можно заявлять и почему применение обосновано.
- [implementation-evidence.md](implementation-evidence.md) — что открыть и показать на защите по каждому пункту ТЗ.
- [defense-speech.md](defense-speech.md) — короткий текст для защиты в формате "Технология / Зачем / Где в коде".
- [project-map/README.md](project-map/README.md) — карта файлов проекта, основные потоки и сложность чтения кода.
- [full-bot-check.md](full-bot-check.md) — чек-лист ручной проверки перед демонстрацией.
- [problems.md](problems.md) — оставшиеся ограничения и задачи.

## 1. Описание сервисов

> Примечание: Profile Service и Matching Service ниже описаны как логические границы системы. В текущем репозитории их функции реализованы внутри Bot Service, а физическое разделение на отдельные REST-сервисы вынесено в список отложенных задач в [problems.md](problems.md).
...
Актуальный список известных проблем, ограничений и отложенных функций вынесен в отдельный файл: [problems.md](problems.md).
