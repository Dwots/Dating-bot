...
- **Что делает:** Просмотр пользователей и их фото, бан/разбан пользователей, статистика (количество пользователей, мэтчей, активность), управление заблокированными аккаунтами
...
sequenceDiagram
    ...
    P->>S3: Сохранить файл
    S3-->>P: s3_key
    P->>DB: Сохранить метаданные фото
    P->>DB: Обновить photo_count и completeness
    P-->>B: "Фото загружено"
    B-->>U: "Фото добавлено в анкету"
```
...
**`photos`** — метаданные фото. Файлы в MinIO, тут только `s3_key`. `is_primary` — главное фото.
...
- **Ручная модерация анкет и фото** — отдельная очередь в админке, где администратор видит новые/жалобные анкеты и фото, принимает или отклоняет их, а результат сохраняется в `moderation_log`.
- **Очередь фото на проверку** — список загруженных фото со статусами `pending` / `approved` / `rejected`. Нужна только если будет включена ручная или автоматическая модерация.
- **Улучшение нагрузочного тестирования** — текущий benchmark нужно заменить или дополнить реальным сценарием нагрузки: JMeter либо согласованный Python/httpx-тест, который обращается к работающим endpoint'ам и сверяет метрики в Prometheus.
- **Расширение CI/CD** — добавить сборку `admin` и `benchmark`, запуск линтера для всех сервисов, базовые тесты и проверку `docker compose config`.
